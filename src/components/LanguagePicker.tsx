...
const LanguagePicker: React.FC<Props> = ({ inverted }) => {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            {open && (
                <div className={`absolute right-0 mt-2 w-44 rounded-md border ${baseBorder} bg-card shadow-lg z-50`} role="listbox">
                    ...
                    <ul className="max-h-64 overflow-auto no-scrollbar text-foreground">
                        {LANGUAGE_OPTIONS.map((opt: LanguageOption) => (
                            <li key={opt.code}>
                                <button
                                    className={`w-full text-left px-3 py-2 text-sm flex items-center gap-2 rounded-md hover:bg-accent text-foreground`}
                                    onClick={() => {
