...
const Logo: React.FC<{ isScrolled: boolean; isHomePage: boolean }> = ({
  isScrolled,
  isHomePage,
}) => {
  const { t } = useT();
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <div className="flex flex-col">
        <span
          className={`font-bold text-2xl ${isHomePage ? (isScrolled ? 'text-foreground' : 'text-white') : 'text-foreground'}`}
        >
          {t('header.brand.title')}
        </span>
        <span
          className={`text-xs -mt-1 ${isHomePage ? (isScrolled ? 'text-muted-foreground' : 'text-white/80') : 'text-muted-foreground'}`}
        >
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="md:hidden">
            <button
              ...
              className={`nav-item outline-none ring-0 ring-offset-0 focus:outline-none focus-visible:outline-none focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0 rounded-md p-1 ${
                isHomePage ? (isScrolled ? 'text-foreground' : 'text-white') : 'text-foreground'
              }`}
        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col space-y-4">
              ...
              <button
                onClick={openContact}
                className="transition-colors bg-primary hover:bg-primary/90 text-white font-semibold rounded-md px-3 py-2 text-left"
              >
                {t('header.nav.contact_sales')}
              </button>
            </div>
